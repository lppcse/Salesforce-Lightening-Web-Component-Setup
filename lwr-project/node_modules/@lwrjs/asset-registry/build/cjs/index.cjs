...
var LwrAssetRegistry = class {
  ...
  async getAsset(assetId, runtimeEnvironment, includeMetadata) {
    ...
    if (transformResult) {
      ...
      import_shared_utils.logger.debug(`[LwrAssetRegistry][getAsset] cached transformed asset ${assetId.specifier}`);
    } else {
  ...
  async resolveAssetUriWithoutLinking(assetId, runtimeEnvironment) {
    ...
    import_shared_utils.logger.debug(`[LwrAssetRegistry][resolveAssetUriWithoutLinking] getUriFromAsset ${specifier} -> ${assetDef.uri}`);
    return uri;
  }
